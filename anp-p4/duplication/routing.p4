...
control MyIngress(inout headers hdr,
    ...
    action ipv4_forward_and_clone(bit<48> dstAddr, bit<9> port){
        hdr.eth_hdr.dstAddr = dstAddr;
        hdr.ip_hdr.ttl = hdr.ip_hdr.ttl - 1;
    ...
    apply {
        if(!hdr.tcp_hdr.isValid() || hdr.ip_hdr.ttl == 0){
            drop();
        }
...
control MyEgress(inout headers hdr,
    ...
    apply { 
        // duplicate packets (recirculating)
        log_msg("Packet on egress with dst port: {}", {standard_metadata.egress_port});
        if(standard_metadata.instance_type == 1) {
            log_msg("Cloned packet! port = {}", {standard_metadata.egress_port});
        } else {
            log_msg("Not a clone");
        }
     }
