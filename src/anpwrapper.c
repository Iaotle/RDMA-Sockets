...
static char *src = NULL,
            *dst = NULL;  // TODO: actually just get this from the passed arguments
...
#pragma endregion
    ...
    // remote_recv_wr.sg_list = &client_recv_sge;
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags) {
    ...
    if (is_anp_sockfd) {
        // rdma_send:
        printf(ANSI_COLOR_YELLOW "rdma_send\n" ANSI_COLOR_RESET);
        ...
        local_mr = rdma_buffer_register(pd, buf, len, (IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_WRITE));
        if (!local_mr) {
        ...
        dst_mr = rdma_buffer_register(pd, dst, len, (IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ));
        if (!dst_mr) {
        ...
        if (check_src_dst(buf, dst, len)) {
            ...
            printf(
                "...\n\033[0;32m SUCCESS, source and destination buffers match "
                "\033[0;37m\n");
        }
...
ssize_t recv(int sockfd, void *buf, size_t len, int flags) {
    ...
    bool is_anp_sockfd = true;
    if (is_anp_sockfd) {
        printf(ANSI_COLOR_YELLOW "rdma_recv\n" ANSI_COLOR_RESET);
        int ret = -1;
        ...
        // 		rdma_get_peer_addr(cm_remote_id) /* gives you remote
        // sockaddr */, 		sizeof(struct sockaddr_in) /* max size
         ...
         * see */
        printf("Remote side buffer information is received...\n");
        show_rdma_buffer_attr(&remote_metadata_attr);
        printf("The remote has requested buffer length of : %u bytes \n", remote_metadata_attr.length);
        ...
        // printf("ok trying to get wc from the remote side write\n");
        // ret = ibv_poll_cq(cq, 1, &wc);
        // printf("ok got: %s, opcode %s\ntrying to get read\n",
        // mapStatusToType(wc.status), mapOpcodeToType(wc.opcode)); ret =
        // ibv_poll_cq(cq, 1, &wc);

        // printf("ok got: %s, opcode %s\n", mapStatusToType(wc.status),
        // mapOpcodeToType(wc.opcode));
...
int close(int sockfd) {
    ...
    if (is_anp_sockfd) {
        // close code
        if (cm_host_id == NULL) rdma_disconnect(cm_host_id);  // hacky way to check if we are server or client (remove)
        struct rdma_cm_event *cm_event = NULL;
        process_rdma_cm_event(cm_event_channel, RDMA_CM_EVENT_DISCONNECTED, &cm_event);
